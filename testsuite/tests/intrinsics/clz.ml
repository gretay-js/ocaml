...
let test_int () =
  (* call directly *)
  assert (int_clz 0 = 63);
  assert (int_clz 1 = (63 - 1));
  assert (int_clz 7 = (63 - 3));
  assert ((int_clz Int.max_int) = 1);
  assert ((int_clz Int.min_int) = 0);
  (* call through standard library routines *)
  assert (Int.count_leading_zeros 0 = 63);
  assert (Int.count_leading_zeros 1 = (63 - 1));
  assert (Int.count_leading_zeros 7 = (63 - 3));
